...
test.describe('Dashboard sub-route navigation', () => {
  ...
  test('shows overview section at /dashboard with breadcrumbs', async ({ page }) => {
    await expect(page.getByText(/Welcome back/)).toBeVisible();
    const breadcrumbs = page.getByRole('navigation', { name: 'Breadcrumb' });
    await expect(breadcrumbs.getByRole('link', { name: 'Dashboard' })).toBeVisible();
    await expect(breadcrumbs.getByRole('link', { name: 'Overview' })).toBeVisible();
  });
  ...
  for (const section of sections) {
    test(`sidebar navigates to ${section.label}`, async ({ page }) => {
      ...
      const breadcrumbs = page.getByRole('navigation', { name: 'Breadcrumb' });
      await expect(breadcrumbs.getByRole('link', { name: section.label })).toBeVisible();

      const placeholder = page.locator('main');
      await expect(placeholder.getByText('Coming soon')).toBeVisible();
      await expect(
        placeholder.getByRole('heading', { name: section.label })
      ).toBeVisible();

      await expect(page.locator('header').getByText('MiniMed')).toBeVisible();
    });
  }
