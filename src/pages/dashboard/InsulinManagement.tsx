import { useState, useEffect } from 'react';
import { useLanguage } from '@/contexts/LanguageContext';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Droplet } from 'lucide-react';

const InsulinManagement = () => {
  const { t } = useLanguage();
  const [translations, setTranslations] = useState({
    title: 'Insulin Management',
    description: 'Review basal rates, bolus deliveries, and insulin settings',
    comingSoon: 'Coming soon',
    placeholder: 'Insulin delivery history and dosing controls will appear here.'
  });

  useEffect(() => {
    const loadTranslations = async () => {
      setTranslations({
        title: await t('Insulin Management'),
        description: await t('Review basal rates, bolus deliveries, and insulin settings'),
        comingSoon: await t('Coming soon'),
        placeholder: await t('Insulin delivery history and dosing controls will appear here.')
      });
    };
    loadTranslations();
  }, [t]);

  return (
    <>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{translations.title}</h1>
        <p className="text-gray-600">{translations.description}</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            <Droplet className="h-5 w-5 text-medtronic-brightBlue" />
            <span>{translations.title}</span>
          </CardTitle>
          <CardDescription>{translations.comingSoon}</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-gray-500">{translations.placeholder}</p>
        </CardContent>
      </Card>
    </>
  );
};

export default InsulinManagement;
