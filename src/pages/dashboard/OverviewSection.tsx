...
import { Activity, Droplet, Zap, ArrowUp, ArrowDown, Minus } from 'lucide-react';
import { generateGlucoseReadings, generateDeviceStatus, generateDailySummary, generateInsulinDeliveries } from '@/utils/mockData';
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">{translations.currentGlucose}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              ...
              <div className="flex flex-col items-center">
                ...
                <span className="mt-1 text-xs text-gray-500">{dailySummary?.currentGlucose.trend}</span>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">{translations.timeInRange}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-3xl font-bold text-green-600">{dailySummary ? Math.round(dailySummary.timeInRange.inRange) : '--'}%</p>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">{translations.totalInsulinToday}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-3xl font-bold text-blue-600">{dailySummary?.totalInsulin || '--'}</p>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-gray-600">{translations.pumpStatus}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  <Progress value={deviceStatus?.batteryLevel} className="h-2 w-20" />
      ...
      <div className="grid grid-cols-1 gap-6 mb-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>{translations.glucoseTrendDesc}</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <AreaChart data={chartData}>
                ...
                <Area type="monotone" dataKey="value" stroke="#3b82f6" fillOpacity={1} fill="url(#colorGlucose)" />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{translations.timeInRangeDesc}</CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie data={timeInRangeData} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{translations.recentActivityDesc}</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {generateInsulinDeliveries(1).slice(0, 5).map((delivery, index) => (
              <div key={index} className="flex items-center justify-between py-2 border-b last:border-0">
                ...
                <span className="text-sm text-gray-500">{format(delivery.timestamp, 'MMM d')}</span>
              </div>
            ))}
          </div>
        </CardContent>
